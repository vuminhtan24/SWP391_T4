                        ...
                        <div class="col-12">
                            <div class="form-card">
                                <div class="form-card-header">
                                    <h4>
                                        <i class="fas fa-plus-circle me-2"></i>Edit Blog Post
                                    </h4>
                                </div>
                                    ...
                                    <div class="form-card-body">
                                                ...
                                                <div class="form-row">
                                                    ...
                                                    <select class="form-select" id="category" name="cid" required>
                                                        ...
                                                        <c:forEach var="category" items="${cList}">
                                                            <option value="${category.categoryId}" ${category.categoryId == b.category.categoryId ? 'selected' : ''}>${category.categoryId}. ${category.categoryName}</option>
                                                        </c:forEach>
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <div class="form-row">
                                            ...
                                            <textarea class="form-control" id="preContext" name="pre_context" rows="3" 
                                                      ...
                                                      maxlength="200" required>${b.pre_context}</textarea>
                                            <div class="character-count">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <textarea class="form-control" id="content" name="context" rows="15" 
                                                      placeholder="" required>${b.context}</textarea>
...
// Initialize existing image preview
                                                const blogImgUrl = '${b.img_url}';
                                                const preview = document.getElementById('imagePreview');
                                                const previewImg = document.getElementById('previewImg');

                                                if (blogImgUrl) {
                                                    previewImg.src = '${pageContext.request.contextPath}/upload/BlogIMG/' + blogImgUrl;
                                                    preview.style.display = 'block';
                                                } else {
                                                    preview.style.display = 'none';
                                                }

// Track if user has changed the image
                                                    ...
                                                    if (file) {
                                                        ...
                                                        if (!file.type.startsWith('image/')) {
                                                            Swal.fire({
                                                                icon: 'error',
                                                                title: 'Invalid File',
                                                                text: 'Only image files are allowed (e.g., JPG, PNG, GIF).',
                                                                customClass: {
                                                                    popup: 'swal-wide'
                                                                }
                                                            });
                                                            ...
                                                            // If there was an existing image, restore it
                                                            if (blogImgUrl) {
                                                                previewImg.src = '${pageContext.request.contextPath}/upload/BlogIMG/' + blogImgUrl;
                                                                preview.style.display = 'block';
                                                            } else {
                                                        ...
                                                        // No file selected - restore existing image if available
                                                        if (blogImgUrl) {
                                                            previewImg.src = '${pageContext.request.contextPath}/upload/BlogIMG/' + blogImgUrl;
                                                            preview.style.display = 'block';
                                                        } else {
                                                ...
                                                function doSubmit() {
                                                    ...
                                                    const preContext = $('#preContext').val().trim();
                                                    ...
                                                    const hasExistingImage = blogImgUrl && blogImgUrl.trim() !== '';
                                                    const hasNewImage = imageInput.files && imageInput.files[0];

                                                    // Blog must have an image (either existing or new)
                                                    if (!hasExistingImage && !hasNewImage) {
                                                        Swal.fire({
                                                            icon: 'warning',
                                                            title: 'Missing Image',
                                                            text: 'Blog post must have a featured image. Please upload an image.',
                                                            customClass: {
                                                                popup: 'swal-wide'
                                                            }
                                                        });
                                                    ...
                                                    // Only append image if user has selected a new one
                                                    ...
                                                    // Submit form
                                                    $.ajax({
                                                        url: '${pageContext.request.contextPath}/blog/edit',
                                                        type: 'POST',
                                                        data: formData,
                                                        processData: false,
                                                        contentType: false,
                                                        success: function (response) {
                                                            if (response.ok) {
                                                                Swal.fire({
                                                                    icon: 'success',
                                                                    title: 'Success!',
                                                                    text: response.message ? response.message : 'Blog Updated Successfully!',
                                                                    timer: 2000,
